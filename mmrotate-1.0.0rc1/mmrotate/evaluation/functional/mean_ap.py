...
def eval_rbbox_map(det_results,
    ...
    FARs.append(sum(FARs)/len(FARs) if len(FARs)>0 else 'nan')
    print_map_summary(
        mean_ap, eval_results, dataset, area_ranges, false_alarm_rate=FARs, logger=logger)
...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets', 'recall', 'ap', 'FAR']
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        for j in range(num_classes):
            row_data = [
                ...
                f'{recalls[i, j]:.3f}', f'{aps[i, j]:.3f}', f'{false_alarm_rate[j]:.3f}'
            ]
            table_data.append(row_data)
        table_data.append(['mAP/mFAR', '', '', '', f'{mean_ap[i]:.3f}', f'{false_alarm_rate[-1]:.3f}'])

...
class EARLRTMDetHead(RTMDetHead):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            if self.train_cfg['spatial_distance_weight']:
                label_weights[pos_inds] = assign_result.get_extra_property('gt_centers')[pos_inds]
            else:
                label_weights[pos_inds] = 1.0
        if len(neg_inds) > 0:
        ...
        for gt_inds in class_assigned_gt_inds:
            ...
            #    gt_class_inds]
            if self.train_cfg['spatial_distance_weight']:
                assign_metrics[gt_class_inds] = assign_result.get_extra_property('gt_centers')[
                    gt_class_inds]
            else:
                assign_metrics[gt_class_inds] = 1.0
...
@MODELS.register_module()
class EARLRTMDetSepBNHead(EARLRTMDetHead):
